...
// Usernames people submit through the form get stored here.
// This is just an array in memory, so it empties out every time the server restarts.
let users = [];
...
const server = http.createServer(function (req, res) {
  ...
  if (pathName === "/users" && method === "GET") {
    fs.readFile(path.join(__dirname, "users.html"), "utf8", function (err, data) {
      ...
      // Build a little list of usernames people have submitted so far
      let extraUsers = "";
