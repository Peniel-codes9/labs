...
For the PR, a short summary like *"Checked GET /, GET /users, empty username, 404, 405, 403, and 500 — all matched expected behavior"* is enough evidence.

## Known limitations

- Submitted usernames are kept in memory only and reset on restart (no database yet — that's a later lab).
